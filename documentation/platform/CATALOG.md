...
```javascript
// Promise
const promise = client.application("<APPLICATION_ID>").catalog.getOptimalLocations({  body : value });

// Async/Await
const data = await client.application("<APPLICATION_ID>").catalog.getOptimalLocations({  body : value });
```
